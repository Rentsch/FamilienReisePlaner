  ...
  return (
    <div className="flex flex-1 flex-col">
      ...
      <main className="mx-auto w-full max-w-2xl flex-1 px-6 py-10">
        ...
        <h1 className="mt-2 mb-1 text-2xl font-semibold text-black dark:text-zinc-50">
          {trip.name}
        </h1>
        {trip.description && (
        ...
        <div className="mb-8 flex flex-wrap items-center gap-3 rounded-xl border border-black/10 p-4 dark:border-white/10">
          <div className="flex-1">
            <p className="text-xs text-zinc-500 dark:text-zinc-400">Share-Link für die Familie</p>
            <p className="break-all font-mono text-sm text-black dark:text-zinc-50">{shareUrl}</p>
          </div>
        ...
        <section className="mb-8">
          ...
          <ul className="flex flex-wrap gap-2">
            ...
            {trip.tripTrailers.map((t) => (
              <li
                key={t.id}
                className="rounded-full border border-dashed border-black/20 px-3 py-1 text-sm text-zinc-700 dark:border-white/20 dark:text-zinc-300"
              >
                {t.trailer.name}
              </li>
            ))}
            ...
            {trip.variants.map((variant) => (
              <li key={variant.id}>
                <Link
                  ...
                  className="flex items-center justify-between rounded-lg border border-black/10 px-4 py-3 text-sm hover:bg-black/[.02] dark:border-white/10 dark:hover:bg-white/[.03]"
                >
                  <span className="text-black dark:text-zinc-50">{variant.name}</span>
